...
{
    public static bool ContainsOnFinalize(this IEnumerable<CustomAttribute> attributes)
    {
        return attributes.Any(x => x.AttributeType.FullName == "DisposeGuard.Fody.OnFinalize");
    }

    public static void RemoveOnFinalize(this Collection<CustomAttribute> attributes)
    {
        var attribute = attributes.FirstOrDefault(x => x.AttributeType.FullName == "DisposeGuard.Fody.OnFinalize");
        if (attribute != null)
        {
